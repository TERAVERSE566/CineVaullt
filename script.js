/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   CINEVAULT â€” Shared Utilities (script.js)
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */

/* â•â• YOUTUBE TRAILER MAP â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
var TRAILERS = {
  'War': 'KKFanta3JJ4',
  'Pathaan': 'vqu4z34wENw',
...
/* â•â• WATCHLIST â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
var watchlist = [];
...
function addToWatchlist() {
  ...
  if (btn) btn.textContent = 'âœ“ In Watchlist';
}
...
function renderWatchlistPanel() {
  ...
  for (var i = 0; i < watchlist.length; i++) {
    ...
    var genreFirst = m.genre.split('Â·')[0].trim();
    html +=
      '<div class="wl-item">' +
      ...
      '<div class="wl-item-meta">â­ ' + m.rating + ' Â· ' + m.year + ' Â· ' + m.duration + '</div>' +
      '<span class="wl-item-genre">' + genreFirst + '</span>' +
      '</div>' +
      '<div class="wl-item-actions">' +
      '<button class="wl-play-btn" onclick="closeWatchlist();openPlayerFromWL(\'' + st + "','" + m.year + "','" + m.duration + "','" + m.rating + "','" + sg + "','" + sd + "','" + m.poster + '\')">â–¶ Play</button>' +
      '<button class="wl-remove-btn" onclick="removeFromWatchlist(\'' + st + '\')">âœ• Remove</button>' +
      '</div>' +
      '</div>';
  }
  list.innerHTML = html;
...
/* â•â• CURRENT MOVIE STATE â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
function openModal(title, year, duration, rating, genre, desc, posterUrl) {
  ...
  if (wlBtn) wlBtn.textContent = isInWatchlist(title) ? 'âœ“ In Watchlist' : 'ï¼‹ Watchlist';
...
/* â•â• VIDEO PLAYER â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
function playMovie() {
  if (!currentMovie) return;
  closeModal();
  setTimeout(function() {
...
function openPlayer(title, year, duration, rating, genre, poster) {
  ...
  document.getElementById('playerMeta').innerHTML =
    '<strong style="color:var(--gold)">â­ ' + rating + '</strong>' +
    ' <span style="opacity:.4">Â·</span> ' + year +
    ' <span style="opacity:.4">Â·</span> ' + duration +
    ' <span style="opacity:.4">Â·</span> ' + genre;
  var wlBtn = document.getElementById('playerWlBtn');
  if (isInWatchlist(title)) {
    wlBtn.textContent = 'âœ“ In Watchlist'; wlBtn.classList.add('added');
  } else {
    wlBtn.textContent = 'ï¼‹ Watchlist'; wlBtn.classList.remove('added');
  }
...
/* â•â• CATEGORY FILTER â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
var GENRE_KEYWORDS = {
  action: ['action'],
  comedy: ['comedy'],
  thriller: ['thriller'],
...
var CAT_LABELS = {
  all: 'ðŸŽ¬ All', trending: 'ðŸ”¥ Trending', bollywood: 'ðŸ‡®ðŸ‡³ Bollywood', hollywood: 'ðŸ‡ºðŸ‡¸ Hollywood',
  action: 'ðŸ’¥ Action', comedy: 'ðŸ˜‚ Comedy', thriller: 'ðŸ”ª Thriller', horror: 'ðŸ‘» Horror',
  drama: 'ðŸŽ­ Drama', 'sci-fi': 'ðŸš€ Sci-Fi', romance: 'â¤ï¸ Romance', crime: 'ðŸ”« Crime'
};
...
function filterByCategory(cat) {
  ...
  for (var f = 0; f < filtered.length; f++) {
    ...
    var bl = r >= 8.5 ? ('â­ ' + fm.rating) : (r >= 7 ? 'HD' : 'FREE');
    ghtml +=
      ...
      '<span class="card-rating">â­ ' + fm.rating + ' Â· ' + fm.year + '</span>' +
      '</div>' +
      '</div>';
  }
...
function runSearch(q) {
  ...
  for (var j = 0; j < results.length; j++) {
    ...
    h += '<div class="search-item" data-idx="' + j + '">' +
      ...
      '<div class="search-item-meta">â­ ' + r.rating + ' Â· ' + r.year + ' Â· ' + r.genre + '</div>' +
      '</div></div>';
  }
...
/* â•â• FADE-IN SECTIONS â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
function initFadeIn() {
...
/* â•â• BROKEN IMAGE FIXER â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
var IMG_COLORS = [
...
function init() {
  ...
  if (playerWlBtn) {
    playerWlBtn.addEventListener('click', function() {
      ...
      showToast('"' + currentMovie.title + '" added');
      playerWlBtn.textContent = 'âœ“ In Watchlist';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(searchInput) {
        searchInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { const query = e.target.value.trim(); if (query.length > 0) window.location.href = 'search.php?q=' + encodeURIComponent(query); } });
