...
$query  = "SELECT id, title, release_year as year, duration, rating, genre, description as `desc`, poster_url as poster FROM content WHERE genre LIKE '%Animation%' OR category = 'anime'";
$params = [];
...
<div class="category-bar" id="categoryBar">
    <div class="cat-chips" id="catChips">
        ...
        ?>
        <button class="cat-chip <?= $active ?>" onclick="window.location.href='anime.php?cat=<?= $key ?>&sort=<?= $sortBy ?>&min_rating=<?= $minRating ?>&year_from=<?= $yearFrom ?>&year_to=<?= $yearTo ?>'"><?= $label ?></button>
        <?php endforeach; ?>
    </div>
</div>
...
<form method="GET" class="adv-filter-bar">
    ...
    <a href="anime.php?cat=<?= $catFilter ?>" style="color:#666;font-size:12px;align-self:center;text-decoration:none">✕ Reset</a>
    <?php endif; ?>
</form>
...
<section class="content-section" data-section="all">
    <div class="section-header">
        <h2 class="section-title">🎌 Anime <span style="font-size:14px;color:#666;font-weight:400">(<?= count($items) ?> titles)</span></h2>
    </div>
    <div class="row" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:20px;padding:0 20px">
        ...
        <div style="grid-column:1/-1;text-align:center;padding:80px;color:#555">
            ...
            <a href="anime.php" style="color:var(--red);text-decoration:none;margin-top:12px;display:inline-block">Clear Filters</a>
        </div>
        <?php endif; ?>
    </div>
</section>
